...
type fakeTaxRuleRepo struct{ rules []withholding.TaxRule }

func (f *fakeTaxRuleRepo) FindApplicable(_ context.Context, taxType withholding.TaxType, conceptID uint, cityID *uint, at time.Time) (*withholding.TaxRule, error) {
...
func (f *fakeTaxRuleRepo) ListByTaxType(context.Context, withholding.TaxType) ([]withholding.TaxRule, error) {
	return nil, nil
}

type fakeReferenceData struct {
	cities map[string]withholding.City
	uvt    *withholding.UVTValue
}

func (f *fakeReferenceData) FindConceptByCode(context.Context, string) (*withholding.Concept, error) {
	return nil, nil
}
func (f *fakeReferenceData) ListConcepts(context.Context) ([]withholding.Concept, error) {
	return nil, nil
}
func (f *fakeReferenceData) FindCityByName(_ context.Context, name string) (*withholding.City, error) {
	c, ok := f.cities[name]
	if !ok {
		return nil, nil
	}
...
func (f *fakeReferenceData) FindUVTValue(context.Context, time.Time) (*withholding.UVTValue, error) {
	return f.uvt, nil
}

type fakeCalculationRepo struct{ saved []withholding.Calculation }

func (f *fakeCalculationRepo) Upsert(_ context.Context, c *withholding.Calculation) error {
	c.ID = uint(len(f.saved) + 1)
	f.saved = append(f.saved, *c)
	return nil
}
func (f *fakeCalculationRepo) ListByInvoice(context.Context, uint) ([]withholding.Calculation, error) {
	return nil, nil
}

const compraBienesConceptID uint = 1

func baseInvoice() *invoice.Invoice {
...
func baseRetefuenteRule() withholding.TaxRule {
...
func baseReteivaRule() withholding.TaxRule {
	return withholding.TaxRule{
...
func findByTaxType(calcs []withholding.Calculation, tt withholding.TaxType) *withholding.Calculation {
	for i := range calcs {
...
func TestCalculateWithholdings_Execute(t *testing.T) {
	uvt := &withholding.UVTValue{Value: decimal.RequireFromString("52374")}

	t.Run("linea supera minimo RETEFUENTE", func(t *testing.T) {
		taxRules := &fakeTaxRuleRepo{rules: []withholding.TaxRule{baseRetefuenteRule()}}
		...
		refuente := findByTaxType(results, withholding.TaxTypeRetefuente)
		if refuente == nil {
	...
	t.Run("ciudad sin tarifa ICA", func(t *testing.T) {
		...
		calcs := &fakeCalculationRepo{}
		uc := usecase.NewCalculateWithholdings(taxRules, calcs, &fakeReferenceData{uvt: uvt, cities: map[string]withholding.City{}}, appconfig.Config{})
		...
		reteica := findByTaxType(results, withholding.TaxTypeReteica)
		if reteica == nil {
	...
	t.Run("comprador no agente de IVA", func(t *testing.T) {
		taxRules := &fakeTaxRuleRepo{rules: []withholding.TaxRule{baseReteivaRule()}}
		calcs := &fakeCalculationRepo{}
		...
		reteiva := findByTaxType(results, withholding.TaxTypeReteiva)
		if reteiva == nil {
